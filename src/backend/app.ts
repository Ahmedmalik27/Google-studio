...
const generateBlogWithGemini = async (apiKey: string, count: number) => {
  ...
  const response = await ai.models.generateContent({
    ...
    config: {
      ...
      responseSchema: {
        type: Type.OBJECT, // Changed to OBJECT to match the schema wrapper if needed, or stick to ARRAY
        properties: {
          posts: {
            type: Type.ARRAY,
            items: {
...
app.get("/api/sitemap.xml", async (req, res) => {
  const primaryPages = [
    ...
    { path: "graphic-design", priority: "0.9", changefreq: "monthly" },
    { path: "google-ads", priority: "0.9", changefreq: "monthly" },
    { path: "ai-agents", priority: "0.9", changefreq: "monthly" }
  ];

  const blogUrls: string[] = [];
  try {
    const projectId = "ai-studio-applet-webapp-154f4";
    const databaseId = "ai-studio-a446ed7f-70ac-4b33-acf2-6caf25f3a985";
    const url = `https://firestore.googleapis.com/v1/projects/${projectId}/databases/${databaseId}/documents/blogs`;
  ...
  // Add primary & service pages
  for (const page of primaryPages) {
    xml += `  <url>\n`;
    xml += `    <loc>https://whalessolution.com/${page.path}</loc>\n`;
  ...
  // Add dynamic blogs
  for (const slug of blogUrls) {
    xml += `  <url>\n`;
    xml += `    <loc>https://whalessolution.com/blog/${slug}</loc>\n`;
    xml += `    <lastmod>${today}</lastmod>\n`;
...
app.post("/api/ai/generate-blog", async (req, res) => {
  ...
  try {
    ...
    const status = (err.status === 429 || err.code === 429 || errorMessage.includes("429")) ? 429 : 
                   (err.status === 503 || err.code === 503 || errorMessage.toLowerCase().includes("overloaded") || errorMessage.toLowerCase().includes("unavailable")) ? 503 : 500;

    res.status(status).json({ 
      error: errorMessage, 
      code: status,
      advice: status === 429 ? "AI Quota exhausted. You can upgrade to a higher tier in the Settings > Secrets panel (Paid Model Flow)." : "",
      details: status === 503 ? "Model is currently overloaded or experiencing high demand. Please retry in a few seconds." : "Operation failed"
    });
  }
});
